...
- Next concrete step: push the verified branch with lease, mark the PR ready, move labels from `agent:claimed` to `agent:needs-qa`, post the runbook delta and DONE comments, then remove the temporary worktree.
