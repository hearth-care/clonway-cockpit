...
- Current phase: verification after takeover; then finish protocol.
...
- Next concrete step: run focused checks plus full gates after this handoff correction, push with lease, mark the PR ready, move labels from `agent:claimed` to `agent:needs-qa`, post the runbook delta and DONE comments, then remove the temporary worktree.
