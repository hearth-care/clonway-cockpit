...
- [x] **Phase 6 (C16):** table restructure + test extension. Files: `docs/persona-platform-architecture.md`, `tests/test_docs_delivery_truth.py`.
- [x] Changelog entries per phase under `[Unreleased]`.
...
- Current phase: Phase 6 (C16) implemented; preparing full `make check` before commit/push.
- Next concrete step: run final full gates, rebase onto latest `origin/main`, push with `--force-with-lease`, then finish protocol.
...
- Known-failing tests: none known after `uv run pytest -q tests/test_docs_delivery_truth.py` passed. `make docs` currently emits pdoc type-annotation warnings for imported/generic symbols but still builds `build/docs`.
