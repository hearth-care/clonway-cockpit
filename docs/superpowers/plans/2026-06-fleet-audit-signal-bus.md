...
- Current phase: QA-fix round (Round-6 FIX+VERIFY, fixer-claude-…4067) — VERIFIED & ready. Implementation complete. Round-6 QA FAIL (qa-claude-225909) had ONE new blocking finding: the R3 read-failure fix's correctness boundary was only half-pinned — the download-*failure* side (raise → no-advance) had a test (POLL-6D), but the must-advance-on-empty side (a successful download that parses to ZERO signals → poll MUST advance past it) had no test anywhere, so the QA could inject `if not out: raise _ArchiveReadError(...)` into `_read_signals`, turning every empty/all-malformed object into a permanent head-of-line block, and the full suite still passed. Fix: added two revert-proven regression tests pinning the must-advance side.
- Next concrete step: complete FIX+VERIFY adversarial re-verification of all prior findings, then mark ready, relabel `agent:qa-passed`, post FIX+VERIFY comment with real gate tails.
...
- **QA-fix (blocking): archive read failures preserve at-least-once.** `subscribe._read_signals` previously returned `[]` when `download_as_text()` raised, making `poll()` treat a failed read as a successfully downloaded empty/malformed object and commit the cursor. A listed-but-unread object could therefore be skipped forever. `_read_signals` now raises an internal sentinel only for download failure; `poll()` breaks that worker's object loop without saving the cursor, so the next healthy poll re-delivers. Regression: `test_poll_download_failure_leaves_cursor_uncommitted`. Malformed lines after a successful download still parse to zero signals and advance as before.
...
- **QA-fix (Round-6 blocking): pin the must-advance-on-empty side of the read boundary.** The R3 fix split `_read_signals` into "download failed → raise `_ArchiveReadError` → `poll` breaks, no advance" vs "downloaded, parsed to zero → return `[]` → `poll` advances past it". Only the *raise* side was tested (POLL-6D); the *advance* side was unpinned, so swapping the raise to key on the empty *result* (`if not out: raise`) — which makes every empty/all-malformed object a permanent head-of-line block — passed the whole suite. Added `test_poll_empty_object_advances_cursor` (POLL-6E, empty body) and `test_poll_all_malformed_object_advances_cursor` (POLL-6F, all lines unparseable under wire-schema drift): each places a real object lexically *behind* the zero-signal object and asserts (a) the real signal is still delivered, (b) the cursor records the zero-signal object's run_id, (c) a second poll re-delivers nothing. Revert-proven: applying the `if not out: raise` injection makes both fail with `assert [] == ['real']` (real signal lost behind the block); on the real code both pass. No source change — `_read_signals`/`poll` were already correct; this round only closes the test gap.
